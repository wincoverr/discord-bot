...
if(message.content.startsWith("eske")){
...
client.on('message', async message => {
    ...
    if(message.content.startsWith(`${PREFIX}bt`)) {
        ...
        const voiceChannel = message.member.voice.channel
                ...
                c=0; //change this later
                console.log(c);
                link=bdd["bt"]["file"][c]
                playmusic(message.member.voice.channel,link);
                //c=0;
                //etat=0;
                //var i;
                // for(i = 0; i < b; i++){
                   // do{
                    //    link=bdd["bt"]["file"][c] //when i put a number like 1 instead of c, i have no error message
                     //   playmusic(message.member.voice.channel,link);
                   // }while (etat=0);
    ...
    }else if (message.content.startsWith(`my hero academia`)) {
        message.channel.send("You found the correct answer")
        etat=1;
    }
})
...
async function playmusic(voiceChannel,link){
   
    if(!voiceChannel) return voiceChannel.send("You need to be in a voice channel to play music")
    try {
        var connection = await voiceChannel.join()
    } catch (error) {
    console.log(`There was an error connecting to the voice channel: ${error}`)
